...
const CartSidebar = () => {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-background/60 z-[60]" onClick={() => setIsOpen(false)} />
      <div className="fixed top-0 end-0 h-full w-full max-w-md bg-card z-[70] flex flex-col border-s border-border shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-border">
          ...
          <button onClick={() => setIsOpen(false)} className="text-foreground hover:opacity-70 transition-opacity">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-muted-foreground font-body">{t("cart.empty")}</p>
              <Link
                to="/store"
              ...
              {items.map((item) => (
                <div key={item.id} className="flex gap-4">
                  <div className="w-20 h-20 bg-secondary overflow-hidden flex-shrink-0">
                    <img src={item.image} alt={lang === "he" ? item.nameHe : item.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="text-xs font-body text-foreground w-4 text-center">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="text-muted-foreground hover:text-foreground transition-colors"
                      >
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-border">
            <div className="flex justify-between items-center mb-4">
              <span className="text-xs font-heading tracking-[0.15em] text-foreground">{t("cart.total")}</span>
              <span className="text-sm font-heading text-foreground">₪{total.toFixed(2)}</span>
            </div>
